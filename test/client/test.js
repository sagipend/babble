...
describe("Save correct details", function () {
  it("Should save name and email to local storage", function () {
    let name = "Sagi Pendzel";
    let email = "sagipend@gmail.com";
    document.getElementById("modal-name").value = name;
    document.getElementById("modal-email").value = email;
    document.getElementById("btnLogin").click();
    let ls = localStorage.getItem('babble');
    assert.equal(JSON.parse(ls).userInfo.name, name, "ERROR in the name");
    assert.equal(JSON.parse(ls).userInfo.email, email, "ERROR in the email");
    localStorage.clear();
  });
});